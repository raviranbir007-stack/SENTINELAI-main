...
async def generate_comprehensive_report(
    ...
    try:
        ...
        for interval_key in request.intervals:
            ...
            avg_apis_checked = sum(s.analysis_data.get("forensic_metadata", {}).get("apis_checked", 0) for s in all_scans if s.analysis_data) / len(all_scans) if all_scans else 0
            ...
            high_confidence_threats = sum(1 for s in all_scans if s.threat_level in ["suspicious", "malicious"] and s.confidence >= 0.7)
...
async def _generate_comprehensive_pdf(report_data: dict, request: AdvancedReportRequest) -> bytes:
    ...
    for interval_key, data in report_data.items():
        ...
        if stats["safe_scans"] > stats["total_scans"] * 0.8:
            recommendations.append(f"✅ <b>Good Security Posture</b>: {stats['safe_scans']/stats['total_scans']*100:.1f}% of scans are clean. Continue monitoring.")
